...
export const getSettings = async (): Promise<AppSettings> => {
  ...
  const keywordConfig = defaults.keywordConfig ?? defaults.keywords;
  const cityList = defaults.cityList ?? defaults.cities;
  if (!keywordConfig || !cityList) {
    throw new Error("Default settings missing keywordConfig/cityList");
  }
