  ...
  import { parseRawEmail, detectCity } from "./lib/parser/emailParser";
  ...
  app.get("/reset-owner", (_req, res) => {
    res.render("reset-owner", { error: null, success: null });
  });

  app.post("/reset-owner", async (req, res) => {
    const { token, email, password } = req.body;
    if (!process.env.RESET_TOKEN || token !== process.env.RESET_TOKEN) {
      return res.render("reset-owner", { error: "Invalid reset token.", success: null });
    }
    if (!email || !password) {
      return res.render("reset-owner", { error: "Email and password are required.", success: null });
    }
    const user = await prisma.user.findUnique({ where: { email } });
    if (!user || user.role !== "OWNER") {
      return res.render("reset-owner", { error: "Owner account not found.", success: null });
    }
    const passwordHash = await hashPassword(password);
    ...
    return res.render("reset-owner", { error: null, success: "Owner password updated." });
  });
  ...
  app.get("/admin/invites", requireOwner, async (_req, res) => {
    ...
    res.render("admin-invites", {
      ...
      users,
      ownerEmail,
      baseUrl: process.env.APP_BASE_URL ?? "https://greenleads.onrender.com"
    });
  });
  ...
  app.post("/import/manual", async (req, res) => {
    const settings = await getSettings();
    ...
    if (!postUrl || !postUrl.includes("nextdoor.com")) {
      return res.render("import", {
        error: "Please provide a valid Nextdoor post URL.",
        success: null,
        settings
      });
    }
    ...
    const resolvedCity =
      city || detectCity(combined, settings.cityList) || null;
    ...
    if (scoreResult.score < settings.threshold) {
      return res.render("import", {
        error: `Score ${scoreResult.score} below threshold ${settings.threshold}.`,
        success: null,
        settings
      });
    }
    ...
    await prisma.lead.create({
      data: {
        source: "nextdoor_manual",
        postUrl,
        title: title || "Nextdoor Lead",
        snippet: snippet || title || "Manual import",
        rawSender: "manual",
        receivedAt: new Date(),
        city: resolvedCity,
    ...
    return res.render("import", {
      error: null,
      success: "Manual lead added successfully.",
      settings
    });
  });
